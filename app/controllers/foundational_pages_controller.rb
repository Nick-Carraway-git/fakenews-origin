class FoundationalPagesController < ApplicationController
  ...
  def results
    ...
    @category_articles = category.to_a.first.articles.page(params[:page]).per(8) if !category.empty?
    @search_articles = Article.where('title like ?', "%#{params[:search]}%").page(params[:page]).per(8)
  end
end
